...
:::note[Running TCAdmin in Docker?]
Updating works the same inside the all-in-one Docker container — the panel and monitor restart by themselves and game servers keep running. See [Updating TCAdmin in Docker](getting-started/Installation/docker-installation.mdx#updating-tcadmin) for details, including the one extra step needed when updating from an older version.
:::
