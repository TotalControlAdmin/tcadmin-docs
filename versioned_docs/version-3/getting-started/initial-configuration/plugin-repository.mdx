...
Repositories sync their content from the Git source automatically. The **Status** column on **Settings** > **Plugin Repositories** tells you where each one stands:

- **Synced** — up to date, everything is good
- **Not Synced** — needs to pull the latest changes (the **Commits** column shows how many commits behind)

A repository that is not synced gets an extra **Sync** button in its **Actions** column. Click it and wait for the status to turn green.
...
Anything you build in TCAdmin — a game, a Docker blueprint, a theme, a layout template, a script or a language pack — can be published back to a plugin repository, either the public community one or a private repository of your own. See [Sharing Plugins](../../using-tcadmin/common/sharing-plugins.mdx).

## What's Next?

- [Sharing Plugins](../../using-tcadmin/common/sharing-plugins.mdx) — publish your own plugins
