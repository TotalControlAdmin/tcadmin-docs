...
Normally, after a Windows server reboots, it sits at the login screen waiting for someone to type in a password. If a game server needs desktop access to run, it can't start until someone logs in. (See [Server Reboots](../../using-tcadmin/server-reboots.mdx) for what happens to your running services during the reboot itself.)
