...
:::tip[Using the Flash Token method? You can usually skip this step]
A remote installed with a deployment token (the recommended method in Step 4) pulls the master's database **and Redis** connection details — passwords included — automatically, and connects to both at whichever master address answered when it joined. Skip straight to [Step 2](#step-2-add-the-remote-server-in-the-panel) unless you need to **pin explicit addresses**: for example, your database lives at a hostname only some machines can resolve, or your remotes reach the database over a dedicated network.
:::
