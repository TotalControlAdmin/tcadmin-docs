...
Before installing TCAdmin, make sure your server meets the requirements below. TCAdmin v3 is built on .NET 10, so it runs on most modern Windows and Linux servers. Windows servers need a 64-bit x86 (x64) processor; Linux servers can be either x64 or 64-bit ARM (ARM64).
...
- **Docker** — only needed if you want to run Docker services (game servers or any other application that ships as a Docker container). You don't have to install it yourself: TCAdmin can install Docker for you from the server's Docker page, with support for Windows containers (Docker's native engine) and Linux containers (Docker Engine inside WSL2). If you prefer to install it manually, use [Docker Engine](https://docs.docker.com/engine/install/) or [Docker Desktop](https://docs.docker.com/desktop/setup/install/windows-install/).
...
:::note[ARM64 servers]
TCAdmin also runs on 64-bit ARM (ARM64) Linux machines — as the master server, as remote servers, or both. Keep in mind that the game servers themselves must also run on ARM: most games only ship x86 builds, so an ARM machine is best suited for games with native ARM64 server builds (such as Minecraft and other Java-based games) or for Docker images built for ARM64.
:::
...
- **Docker** — only needed if you want to run Docker services (game servers or any other application that ships as a Docker container). You don't have to install it yourself: TCAdmin can install [Docker Engine](https://docs.docker.com/engine/install/) for you from the server's Docker page.
