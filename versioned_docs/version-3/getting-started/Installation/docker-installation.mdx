...
The panel's server **Reboot** action works in Docker too — it restarts the **container** (a container has no operating system of its own to reboot). Game servers that were running come back automatically once the container is up again, just like after a real server reboot. See [Server Reboots](../../using-tcadmin/server-reboots.mdx) for what happens to those services during the restart, and why the container needs a longer stop timeout.
...
A web panel container connects to the master's database and Redis directly, exactly like a remote server does. On a different machine, publish `3306` and `6379` on the master (see [Ports](#ports)) and open them **only** to the web machine. The configuration takes care of itself: the token delivers the database and Redis connection details, passwords included, and the web container connects to both at **whichever master address answered its join request**. Follow [the Pin Explicit Addresses section of Remote Server Configuration](../initial-configuration/remote-server-configuration.mdx#pin-explicit-addresses-optional) only if you need to pin explicit addresses — for example, a database on its own network. (You do **not** need Step 2's deployment token — the web panel token below is a different token.)
:::
