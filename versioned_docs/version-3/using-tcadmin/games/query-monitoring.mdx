...
TCAdmin queries each running game service on a schedule. When a check fails, TCAdmin can **restart**, **stop**, or **disable** the service automatically, **email the service's owner**, and (optionally) write an entry to the activity log so there's a record of what happened.

Monitoring is **off by default**. You turn it on per game from the game blueprint's **Monitoring** page.
...
Each detection has its own on/off switch and its own action, so you can (for example) restart on a crash but only log a slot overage — and each can email the service's owner (see [Owner emails](#owner-emails)).

## Settings

On the game blueprint's **Monitoring** page, **Query Failure** tab:
...
| **Failure action** | The action taken below the threshold. Default: Restart + Send email. |
| **Max failure action** | The action taken once the threshold is reached, so a server that keeps failing isn't restart-looped forever. Default: Disable + Send email. |
| **Log activity on failure** | Write an activity-log entry each time a detection fires. Default: on. |

An action is a small set of choices: at most one power action — **Restart**, **Stop**, or **Disable** — optionally combined with **Send email**. Selecting *Send email* reveals a template picker for that detection (see [Owner emails](#owner-emails)).

### Per-service settings

Some options live on the individual service's settings page, in the **Monitoring** section:

- **Disable query monitoring** — exclude this one service from query monitoring. Does not affect automatic restarts after an unexpected exit.
...
On the **Unexpected Exit** tab, TCAdmin can restart a game server that exits on its own — a crash, a quit from the console or RCON, or a kill — without waiting for a query check to notice.

| Setting | What it does |
|---|---|
| **Restart automatically after an unexpected exit** | Turns the feature on. Default: on. |
| **Restart delay (seconds)** | How long TCAdmin waits before restarting. Default: 5. |
| **Maximum restarts** | How many restarts are allowed within the window. Default: 3. |
| **Within (minutes)** | The sliding window the restart count is measured against. Default: 5. |
| **Log unexpected exits to the activity log** | Writes an activity entry for each exit and for a reached limit. Default: on. |

Once the maximum is reached within the window, TCAdmin leaves the server stopped and writes a **Restart Limit Reached** activity entry instead of restarting again.

This tab works independently of the Query Failure tab's **Enabled** switch and the per-service **Disable query monitoring** setting — automatic restart runs even when query monitoring is off.

## Owner emails

Any detection's action can include **Send email**, which notifies the service's owner. Each detection has a matching built-in email template:

| Detection | Template used |
|---|---|
| Query failure (below the threshold) | Server query failure |
| Query failure (threshold reached) | Server maximum query failures |
| Slot overage | Slot limit exceeded |
| Went public | Private status detection |
| Branding removed | Server branding removed |
| Custom rule | Monitoring rule triggered |

When *Send email* is selected, a picker appears next to the action. Keep the default template, or choose a **variant** of it — a copy created on the [Email Templates](../../customizations/email-templates.mdx) page with *Based on* — so different games can send differently worded emails for the same event.

Emails are sent once per event, not on every check, so an owner's inbox isn't flooded while a problem persists:

- A failing server emails once when the failure is first confirmed, and once more if it reaches the failure threshold.
- The other detections email once per occurrence. If the server is restarted and the problem is still there, that's a new occurrence and the email is sent again — one per server run.

:::note
Turning a template off on the Email Templates page stops that email for **every** game, even where *Send email* is selected. Every email also lands in the owner's in-app **Messages** inbox — including when outbound mail isn't set up yet.
:::
...
| `MonitoringAction` | `EQueryMonitoringAction` | The action set TCAdmin chose: `None`, or at most one of `Restart` / `Stop` / `Disable`, optionally combined with `SendEmail` (it's a `[Flags]` enum, so the value can be a combination). |
...
:::note
In a C# script the enum variables read as their name when you call `.ToString()` (for example `Variables["DetectionType"].ToString()` returns `"QueryFailure"`). A flag set reads as the combined names — `Variables["MonitoringAction"].ToString()` returns `"Restart, SendEmail"` when both are selected.
:::
