...
Every variable — the full `Service.*` field list (including the click-to-connect `Service.ConnectUrl`, the blueprint artwork, and where the service runs: server, datacenter and region), the `Service.Ports` entries, and every `Permissions.*` / `Features.*` boolean — is enumerated in the [Layout Template Reference](./reference.mdx#service-page-variables).
