...
The monitoring templates only send for games whose query-monitoring action includes **Send email**, configured per game on its [Monitoring](../using-tcadmin/games/query-monitoring.mdx#owner-emails) settings. Turning a template off on the Email Templates page stops it for every game.
:::
