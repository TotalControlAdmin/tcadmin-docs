...
:::tip
To quickly browse all available TCAdmin objects, type `Globals.` in the code editor or press `Ctrl+Space` to trigger IntelliSense autocompletion.
:::
...
Since `Globals` is the implicit scope, the `Globals.` prefix is **optional** — you can access any property directly by name. Both styles below are equivalent:

```csharp
var Globals = new TCAdmin.Scripting.Engines.Addons.CSharpGameGlobals();

// With the Globals prefix
var server = Globals.ThisServer;
var service = Globals.ThisService;
Globals.ScriptConsole.WriteLine($"Service: {service.Name}");

// Without the prefix (equivalent)
var server = ThisServer;
var service = ThisService;
ScriptConsole.WriteLine($"Service: {service.Name}");
```
