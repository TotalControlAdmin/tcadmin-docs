...
## Refuse to Save a Config File While the Service Is Running

This script runs on the `BeforeServiceConfigFileWritten` event and refuses the write while the server is up, so nobody overwrites a file the running server will clobber or reload mid-session. Administrators are still let through. Add the event to the script and attach the script to the game or Docker blueprint.

`ThisUser` is `null` when the operation has no user behind it, so it is always null-checked. Setting `Globals.ReturnValue` to boolean `false` denies the write — see [Config files](./script-events.mdx#config-files) for what the user sees on each surface.
...
else if (ThisService.Status == EServiceStatus.Started)
{
...
:::warning
Saving a config file also fires `BeforeServiceConfigFileRead` (the panel reads the current file first) as well as `BeforeServiceConfigFileWritten`. A script that denies reads or writes therefore blocks saves of that file too.
:::

## Restart the Service After a Config File Changes

This script runs on the `AfterServiceConfigFileWritten` event. It records who changed the file and how, then restarts the service so the new settings take effect. The event fires after the file is on disk and its variables have been synced, no matter which surface wrote it — the panel, FTP, SFTP or an upload.
...
// Only restart for the settings file we care about.
