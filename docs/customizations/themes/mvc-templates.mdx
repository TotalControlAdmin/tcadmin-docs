...
Https is not supported by the built in web server when `TCAdmin.Public.MVC` is set to True. To use https you must [**configure your control panel with Nginx**](#).
:::
...
Configure Nginx with these instructions [Configure TCAdmin to run with Nginx](#)

## Windows IIS configuration (optional) 

Configure the website in IIS with these instructions: [Configure TCAdmin to run with IIS](#)
