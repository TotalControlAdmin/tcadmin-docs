...
This tool runs the steam update on your base game files and creates a zip or tar with the new/updated files. After the file is created it adds the update to the game's settings. Use the [Batch Update tool](#) to apply it on all services or allow your clients to install it themselves. hldsupdatetool and steamcmd are supported.
