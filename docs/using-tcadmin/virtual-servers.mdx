...
- [Linux Quotas](#) must be enabled on the file systems where virtual server's space will be located. This is only required if you want to limit a virtual server's disk space.
- [schedutils](#) must be installed to set process affinity.
