...
The service manager is in charge of starting the game/voice services automatically after restarting the server. It also receives start/stop commands from the monitor. You can also use the [service browser](/using-tcadmin/service-browser.mdx) to manage the services from remote desktop or ssh.
